    ...
    while (detailmonth.length > getprev) {
        if (detailmonth[getprev].slice(4, 7) === monthlist[monthact]) {
            ...
            if (dateact.slice(0, 10) == temptoday) {
                ...
                document.querySelector(`.weeknb${weeknb}`).classList.add('selectedweek')
            } else {
...
async function showcalendar(weekToDisplay) {
    if (!eventArray.hasOwnProperty(weekToDisplay)) {
        const result = await fetch(`https://apiade.chavanel.eu.org/?q=${noressource}&json=true&weekNumber=${weekToDisplay}&groupBy=day`);
...
function displayweek(weekToDisplay) {


    for (const key in eventArray[weekToDisplay]) {
        if (eventArray[weekToDisplay].hasOwnProperty(key)) {
            // Accessing the events array for each key
            const events = eventArray[weekToDisplay][key].events;
            // Looping through the events array
            for (let i = 0; i < events.length; i++) {
                events[i].start = new Date(events[i].start);
                events[i].end = new Date(events[i].end);

            }
            displayday(weekToDisplay, key);
        }
    }
...
function displayday(weekToDisplay, dateoftheday) {
    ...
    eventArray[weekToDisplay][dateoftheday].events.forEach(element => {
        let nomprof = element.teacher
        let emplacement = element.location

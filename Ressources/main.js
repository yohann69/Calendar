...
=======
    let currentWeek = d.getWeek();
>>>>>>> Stashed changes

    const result = await fetch(`https://calendar-backend-production-c04d.up.railway.app/?q=${noressource}&weekNumber=${currentWeek}&&json=true&groupBy=day`);
    eventArray = (await result.json());
...
=======
    // if (testcalendar[0].includes("BEGIN:VCALENDAR")) {
        ...
        // Add calendar Name
        await getCalendarName(noressource);
...
async function getCalendarName(noressource) {
    let name = await fetch(`https://calendar-backend-production-c04d.up.railway.app/searchName?q=${noressource}`);
